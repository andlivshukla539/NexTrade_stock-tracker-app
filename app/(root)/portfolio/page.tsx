...
async function getPortfolioData(userId: string) {
    ...
    // Simulate current price fetching (or use real API if feasible)
    // For this demo, we'll try to fetch real prices or fallback to a variation
    const holdings = await Promise.all(holdingsDocs.map(async (doc) => {
        let currentPrice = doc.avgPrice;
        try {
            const res = await fetch(`https://finnhub.io/api/v1/quote?symbol=${doc.symbol}&token=${process.env.NEXT_PUBLIC_FINNHUB_API_KEY}`, { next: { revalidate: 60 } });
...
export default async function PortfolioPage() {
    ...
    const totalCost = holdings.reduce((acc, h) => acc + (h.quantity * h.avgPrice), 0);
        ...
        <div className="container mx-auto p-6 space-y-6">
            <div className="flex justify-between items-center">
                ...
                <div className="text-right">
                    ...
                    <p className="text-2xl font-bold text-green-400">${balance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card className="bg-gray-800 border-gray-700">
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold text-gray-100">${(totalValue + balance).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                    </CardContent>
                </Card>
                <Card className="bg-gray-800 border-gray-700">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-gray-400">Invested Value</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold text-gray-100">${totalValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
                    </CardContent>
                </Card>
                <Card className="bg-gray-800 border-gray-700">
                    <CardHeader className="pb-2">
                    ...
                    <CardContent>
                        <div className={`text-2xl font-bold ${totalPnL >= 0 ? "text-green-500" : "text-red-500"}`}>
                            ${Math.abs(totalPnL).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ({totalPnLPercent.toFixed(2)}%)
                        </div>
                    </CardContent>
                </Card>
            </div>
